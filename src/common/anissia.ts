...
class Anissia {
    ...
    public ymdOrDynamicAgo(isoDate: string): string {
        return this.formatOrDynamicAgo(isoDate, 'yyyy-MM-dd');
    }
    ...
    public formatOrDynamicAgo(isoDate: string, format: string): string {
        if (isoDate == null) {
            return '';
        }

        const date = new DateFormat().parseIsoDate(isoDate).toDate();
